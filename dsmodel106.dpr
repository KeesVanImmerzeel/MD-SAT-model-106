library dsmodel106;
  {-Metamodel voor de N-belasting van het ondiepe grondwater, o.a. gebaseerd op
    de Gt. SC-dlo Technisch Document 61, p. 43-45 (1999)}
...
Const
  cModelID      = 106;  {-Uniek modelnummer}
  ...
  cNrXIndepTblsInEP0 = 9;    {-Aantal XIndep-tables in EP[cEP0]}
...
var
  Gt, Bodem, Gewas: Integer;      {-Sleutel-waarden voor de default-tabellen in EP[cEP0]}
  Kwwz7, Kwel7, NOver, NconcKwel, {-Parameter-waarden afkomstig van de Shell}
...
Function SetKeyAndParValues( var IErr: Integer ): Boolean;
  ...
  Function GetGt( const x: Double ): Integer;
  begin
    with EP[ indx-1 ].xDep do
      Result := Trunc( Items[ cTb_Gt ].EstimateY( x, Direction ) );
  end;
  ...
  Function GetDefaultPar1( const Gt: Integer ): Double;
  begin
    with EP[ cEP0 ].xInDep.Items[ cTb_DefaultPar1 ] do
      Result := GetValue( 1, Gt ); {row, column}
  end;
...
begin {-Function SetKeyAndParValues}
  ...
  Gt := GetGt( x );
  if ( Gt < cMin_Gt ) or ( Gt > cMax_Gt ) then begin
    IErr := cInvld_Gt; Exit;
  end;
  ...
  with EP[ cEP0 ].xInDep.Items[ cTb_DefaultPar1 ] do begin
    c             := GetValue( 1, 1 ); {row, column}
    c_Nover       := GetValue( 1, 2 );
    c_Nkwel       := GetValue( 1, 3 );
    c_kwwz7       := GetValue( 1, 4 );
    c_Nover_kwwz7 := GetValue( 1, 5 );
  end;
  ...
  with EP[ cEP0 ].xInDep.Items[ cTb_DefaultPar3 ] do begin
    ...
    c_bodem_gewas  := GetValue( Bodem, 2 + Gewas );
  end;

  with EP[ cEP0 ].xInDep.Items[ cTb_DefaultPar4 ] do
    c_Gt := GetValue( 1, Gt ); {row, column}

  with EP[ cEP0 ].xInDep.Items[ cTb_DefaultPar5 ] do
    c_Gt_gewas := GetValue( Gewas, Gt ); {row, column}

  with EP[ cEP0 ].xInDep.Items[ cTb_DefaultPar6 ] do
    c_Gt_bodem := GetValue( bodem, Gt ); {row, column}
...
begin
  ...
  Application.MessageBox( PChar(
    ...
    'c_Gt: ' + FloatToStr( c_Gt ) + #10 +
    'c_Gt_bodem: ' + FloatToStr( c_Gt_bodem ) + #10 +
    'c_Gt_gewas: ' + FloatToStr( c_Gt_gewas ) + #10 +
  ...
  {$endif}
    ...
    dydx[ cLogNopp ] := c + c_bodem + c_gewas + c_bodem_gewas +
	                c_Gt + c_Gt_bodem + c_Gt_gewas + c_Nkwel * Nkwel +
                        ( c_kwwz7 + c_Nover_kwwz7 * Nover ) * Kwwz7 +
...
begin
  with EP[ cEP0 ].xInDep.Items[ cTb_MinMaxValKeys ] do begin
    cMin_Gt        := Trunc( GetValue( 1, 1 ) ); {rij, kolom}
    cMax_Gt        := Trunc( GetValue( 1, 2 ) );
